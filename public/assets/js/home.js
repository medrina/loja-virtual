        ...
        if(e.target.value != 0) {
            ...
            $.ajax({
                ...
                success: (dados) => {
                    ...
                    let html = `<div class="row"><div class="col-12">${(e.target.value == '2-1') ? '<b>Maior Preço</b>' : '<b>Menor Preço</b>'}</div>`
...
function marca(id, id_subcategoria) {
    $.ajax({
        type: "GET",
        crossDomain: true,
        url: `/marca/produtos`,
        ...
        success: (dados) => {
            dados = JSON.parse(dados)
            let html = `<div class="row"><div class="col-xl-12 col-lg-12 col-sm-12 col-12"><b>Marca:</b> ${dados[0].marca}</div>`
            dados.forEach(element => {
                let precoTotalFormatado = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(element.valor)
                let precoParcelaFormatado = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(element.valor_parcela)
                html += `<div class="col-xl-3 col-lg-4 col-sm-6 col-6">
