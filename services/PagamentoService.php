<?php
    ...
    class PagamentoService extends ItensCarrinhoAux {
        ...
        private function salvarCompraBanco(stdClass $objPgto): string {
        ...
        // setar o Frete
        private function setFrete(): string {
        ...
        // setar o Pedido
        private function setPedido(): string {
        ...
        // setar a Entrega
        private function setEntrega(int $id_frete, int $id_pedido): string {

            // recupera o objeto 'forma_pgto' que está no formato json
            $formaPgto = $_SESSION['forma_pgto'];

            // converte o objeto 'forma_pgto' para formato "objeto dinâmico"
            $formaPgto = json_decode($formaPgto);

            // recupera o objeto frete que está serializado
            $frete = $_SESSION['frete'];

            // desserializa o objeto frete, e converte para formato "objeto dinâmico"
        ...
        // seta os dados do Cartão de Crédito
        private function setCartaoCredito(): string {
            $formaPgto = $_SESSION['forma_pgto'];
            $formaPgto = json_decode($formaPgto);
            try {
        ...
        private function setBoleto($dataVenc): string {
        ...
        // seta os dados do Pix
        private function setPix(): string {
            $formaPgto = $_SESSION['forma_pgto'];
            $formaPgto = json_decode($formaPgto);
            try {
        ...
        private function setPagamento(int $id_tipo_tabela, int $id_entrega): string {
            $formaPgto = $_SESSION['forma_pgto'];
            $formaPgto = json_decode($formaPgto);
            $status = 'status';
            try {
        ...
        // setar os dados da fatura
        private function gerarFatura(int $id_pagamento, int $id_entrega, int $id_pedido): array {
            $array = [];
            try {
                $query = "SELECT 
